...
import com.nbb.models.LutValidation;
import com.nbb.models.StsCheckVariable;
...
public class UserDaoImpl  extends JdbcDaoSupport implements UserDao {
	...
	@Override
	public void inserBatch(List<?> formVal, String validation) {
		if(validation.equals("validation")){
			String sql = "INSERT INTO lut_validation " + "(code1,code2,balanceid,isformula1,isformula2,position1,position2,title1,title2,valid,userid) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
			getJdbcTemplate().batchUpdate(sql, new BatchPreparedStatementSetter() {
				public void setValues(PreparedStatement ps, int i) throws SQLException,NullPointerException {
					LutValidation item = (LutValidation) formVal.get(i);
					ps.setString(1, item.getCode1());
					ps.setString(2, item.getCode2());
					ps.setLong(3, item.getBalanceid());
					ps.setLong(4, item.getIsformula1());
					ps.setLong(5, item.getIsformula2());
					ps.setString(6, item.getPosition1());
					ps.setString(7, item.getPosition2());
					ps.setString(8, item.getTitle1());
					ps.setString(9, item.getTitle2());
					ps.setLong(10, item.getValid());
		...
		else if(validation.equals("tulgalt")){
			String sql = "INSERT INTO STS_CHECK_VARIABLE " + "(ORGCODE, STEPID,PLANID,ORGCATID,DATA1,DATA2,DATA3,DATA4,DATA5,DATA6,DATA7,DATA8,DATA9,DATA10,VALID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			getJdbcTemplate().batchUpdate(sql, new BatchPreparedStatementSetter() {
				public void setValues(PreparedStatement ps, int i) throws SQLException,NullPointerException {
					StsCheckVariable item = (StsCheckVariable) formVal.get(i);
