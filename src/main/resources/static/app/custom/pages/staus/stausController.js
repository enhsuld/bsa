	...
	/*		$scope.update = function(item){
			...
			$scope.treelistOptions = {
				dataSource: {
					transport: {
						read:  {
							url: "http://119.40.98.169:8088/core/tree/list/LutForms",
			...
			$scope.deleteO = function(id){
				...
				closeOnCancel: false
				 ...
				 if (inputvalue) {
					 ...
					 mainService.withdata('POST', 'http://119.40.98.169:8088/core/delete/'+$scope.domain,  $scope.formdata).
						then(function(data){
							$(".k-grid").data("kendoGrid").dataSource.read(); 
			...
			drop   = UIkit.uploadDrop($("#file_upload-drop"), settings);
			  ...
			  $scope.fileGrid = {
					 dataSource: {
						 ...
						 transport: {
							 read:  {
								 url: "http://119.40.98.169:8088/core/list/LutStaus",
								 contentType:"application/json; charset=UTF-8",     
								 data: {"sort":[{field: 'id', dir: 'desc'}]},
								 type:"POST"
							 },
