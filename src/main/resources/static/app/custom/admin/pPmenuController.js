angular
    .module('altairApp')
    .controller("menuCtrl", ['$rootScope', '$scope', 'p_menu', 'user_data', 'mainService', '$state', 'sweet', '__env',
        function ($rootScope, $scope, p_menu, user_data, mainService, $state, sweet, __env) {
            ...
            $scope.edit = function (id) {
                mainService.withdomain('get', __env.apiUrl() + '/core/api/LutMenu/' + id).then(function (data) {
            ...
            $scope.submitForm = function ($event) {
                ...
                if ($scope.formdata.id == null) {
                    mainService.withdata('POST', __env.apiUrl() + '/core/create/' + $scope.domain, $scope.formdata).then(function (data) {
                        modalUpdate.hide();
                    ...
                    mainService.withdata('POST', __env.apiUrl() + '/core/update/' + $scope.domain, $scope.formdata).then(function (data) {
                        modalUpdate.hide();
                        $event.preventDefault();
                        sweet.show('Мэдээлэл', 'Үйлдэл амжилттай.', 'success');
                        $(".k-grid").data("kendoGrid").dataSource.read();
            ...
            $scope.deleteO = function (id) {
                ...
                sweet.show({
                    ...
                    if (inputvalue) {
                        ...
                        mainService.withdata('POST', __env.apiUrl() + '/core/delete/' + $scope.domain, $scope.formdata).then(function (data) {
                            $(".k-grid").data("kendoGrid").dataSource.read();
            ...
            $scope.pmenuGrid = {
                dataSource: {
                    ...
                    transport: {
                        read: {
                            url: __env.apiUrl() + "/core/list/LutMenu",
                ...
                columns: [
                    ...
                    {template: kendo.template($("#update").html()), width: 110}
                ],
                dataBound: function () {
                    var rows = this.items();
                    $(rows).each(function () {
